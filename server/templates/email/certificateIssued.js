const certificateIssuedTemplate = ({
  firstName,
  courseName,
  certificateId,
  downloadUrl,
}) => `
<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
  <style>
    body { margin: 0; padding: 0; background: #0F0F14; font-family: Arial, sans-serif; }
    .container { max-width: 600px; margin: 0 auto; background: #16161F; border: 1px solid #2A2A3A; border-radius: 12px; overflow: hidden; }
    .header { background: linear-gradient(135deg, #1a1238, #3D3F6E); padding: 40px 30px; text-align: center; }
    .logo { font-size: 24px; font-weight: 800; color: #818CF8; }
    .body { padding: 40px 30px; text-align: center; }
    .medal { font-size: 64px; margin-bottom: 16px; }
    .title { font-size: 24px; font-weight: 700; color: #E8E8F0; margin-bottom: 16px; }
    .text { font-size: 15px; color: #9999B8; line-height: 1.7; margin-bottom: 16px; }
    .cert-box { background: linear-gradient(135deg, #1a1238, #3D3F6E); border: 1px solid #818CF844; border-radius: 12px; padding: 24px; margin: 20px 0; }
    .cert-course { font-size: 18px; font-weight: 700; color: #818CF8; margin-bottom: 8px; }
    .cert-id { font-size: 12px; color: #6B6B8A; font-family: monospace; }
    .btn { display: inline-block; background: #818CF8; color: #fff; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: 700; font-size: 15px; margin: 8px; }
    .btn-ghost { display: inline-block; background: transparent; color: #818CF8; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: 700; font-size: 15px; margin: 8px; border: 1px solid #818CF8; }
    .footer { padding: 20px 30px; text-align: center; border-top: 1px solid #2A2A3A; }
    .footer-text { font-size: 12px; color: #6B6B8A; }
  </style>
</head>
<body>
  <div class="container">
    <div class="header">
      <div class="logo">⚡ DEVAD TECH ACADEMY</div>
    </div>
    <div class="body">
      <div class="medal">🏅</div>
      <div class="title">Your Certificate is Ready!</div>
      <p class="text">Congratulations ${firstName}! You've successfully completed:</p>
      <div class="cert-box">
        <div class="cert-course">${courseName}</div>
        <div class="cert-id">Certificate ID: ${certificateId}</div>
      </div>
      <p class="text">Download your certificate and share it on LinkedIn to showcase your achievement!</p>
      <a href="${downloadUrl}" class="btn">Download Certificate</a>
      <a href="${process.env.CLIENT_URL}/certificates/${certificateId}" class="btn-ghost">View Online</a>
    </div>
    <div class="footer">
      <p class="footer-text">© ${new Date().getFullYear()} Devad Tech Academy. All rights reserved.</p>
    </div>
  </div>
</body>
</html>
`;

export default certificateIssuedTemplate;