...
import { changePasswordValidator } from "../../validators/auth/passwordValidator.js";

export const getProfile = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user._id);
  const student = await Student.findOne({ user: req.user._id });
  sendResponse(res, 200, "Profile retrieved", { user, student });
});
...
export const getPublicProfile = asyncHandler(async (req, res) => {
  const user = await User.findOne({
    $or: [
      ...
      { referralCode: req.params.username },
    ],
  }).select("firstName lastName avatar headline bio socialLinks role");

  if (!user) return sendResponse(res, 404, "Profile not found");
  sendResponse(res, 200, "Public profile retrieved", user);
});